...
export function ReviewPage() {
      ...
      <div className="min-h-screen bg-bg">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="noren-card p-12 text-center">
            <CheckCircle2 className="w-16 h-16 text-pine mx-auto mb-4" />
            <h1 className="text-h1 md:text-2xl font-bold text-ink mb-2">太棒了！</h1>
            <p className="text-base text-ink-soft mb-6">今天没有需要复习的语法点</p>
      ...
      <div className="min-h-screen bg-bg">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="noren-card p-8">
            <h1 className="text-h1 md:text-2xl font-bold text-ink mb-6 text-center">复习完成！</h1>

            <div className="bg-pine/10 rounded-md p-6 mb-6 text-center">
              <p className="text-4xl font-bold text-pine mb-2">
            ...
            <div className="space-y-2 mb-6">
              <h2 className="font-semibold text-ink mb-2">已更新复习计划:</h2>
              {completedStats.updates.map((update, index) => {
                ...
                return (
                  <div key={index} className="flex items-center justify-between bg-bg-warm rounded-md p-3">
                    <span className="text-ink">{update.grammarId}</span>
    ...
    return (
      <div className="min-h-screen bg-bg">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {/* Grammar info */}
          <div className="noren-card p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-h1 text-ink">{currentItem.grammarPoint}</h2>
          ...
          {selectedMode === 'browse' ? (
            <div className="noren-card p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-semibold text-ink">快速浏览</h3>
              </div>

              <div className="bg-bg-warm rounded-md p-6 mb-6">
                <p className="text-h1 text-ink mb-4">{currentSentence.sentence}</p>
                <p className="text-ink-soft">{currentSentence.translation}</p>
              </div>
            ...
            <div className="noren-card p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-semibold text-ink">测试模式</h3>
              </div>

              {showResult ? (
                <div className={`text-center py-8 rounded-md ${lastAnswerCorrect ? 'bg-pine/10' : 'bg-accent/10'}`}>
                  <p className={`text-2xl font-bold mb-2 ${lastAnswerCorrect ? 'text-pine' : 'text-accent'}`}>
                    {lastAnswerCorrect ? '✓ 正确！' : '✗ 不正确'}
                  </p>
                  {!lastAnswerCorrect && (
                    <p className="text-ink">正确答案是: {currentItem.grammarPoint}</p>
                  )}
                </div>
              ) : (
                <>
                  <div className="bg-bg-warm rounded-md p-6 mb-6">
                    <p className="text-h1 text-ink mb-4">
                      {currentSentence.sentence.replace(new RegExp(currentItem.grammarId, 'g'), '_____')}
                    </p>
                    <p className="text-ink-soft mb-2">请选择正确的语法点填入空白处:</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {quizOptions.map((option) => (
                      <button
                        ...
                        className="p-4 rounded-md border border-border hover:border-accent hover:bg-surface-hover text-left transition-all"
                      >
                        {option}
                      </button>
                    ))}
          ...
          <button
            ...
            className="mt-6 w-full text-ink-soft hover:text-accent text-xs"
          >
            退出复习
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-bg">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-h1 md:text-3xl font-bold text-ink mb-2">复习</h1>
        <p className="text-base text-ink-soft mb-8">今天有 {reviewItems.length} 个语法点需要复习</p>

        {/* Review list */}
        <div className="noren-card mb-6 overflow-hidden">
          <div className="divide-y divide-border">
            {reviewItems.map((item) => (
              <div key={item.grammarId} className="p-4 flex items-center justify-between hover:bg-surface-hover">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <button
            ...
            className="noren-card p-6 text-left hover:scale-[1.02] transition-all"
          >
            <BookOpen className="w-8 h-8 text-ink mb-3" />
            <h3 className="font-bold text-ink mb-2">快速浏览</h3>
            <p className="text-xs text-ink-soft">
              快速过一遍例句，加深印象。适合时间紧张时使用。
            </p>
            <div className="mt-4 text-ink font-medium flex items-center gap-1">
          ...
          <button
            ...
            className="noren-card p-6 text-left hover:scale-[1.02] transition-all"
          >
            <Brain className="w-8 h-8 text-ink mb-3" />
            <h3 className="font-bold text-ink mb-2">测试模式</h3>
            <p className="text-xs text-ink-soft">
